...
async function init_chat() {
    ...
    chatInput.addEventListener('keypress', async (event) => {
        if (event.key === 'Enter') {
            ...
            const answer = await askGPT("You are a financial assistant.", userInput);
            chatbotText.innerText = answer;
        }
    });
}

async function askGPT(system, prompt) {
    const response = await fetch("/gpt", {  // ✅ 변경된 포트 적용
        method: "POST",
        headers: {
