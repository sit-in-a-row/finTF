...
def load_json(path):
    ...
    # return data
    """
    주어진 경로의 JSON 파일을 읽어 Python dict로 반환하는 함수.
    
    :param path: 읽고자 하는 JSON 파일의 경로
    :return: JSON 파일의 내용을 담은 dict 객체
    """
    try:
        ...
        print(f"Error: 파일 {path}을(를) 찾을 수 없습니다.")
        return {}
    except json.JSONDecodeError:
        print(f"Error: 파일 {path}은(는) 올바른 JSON 형식이 아닙니다.")
        return {}
