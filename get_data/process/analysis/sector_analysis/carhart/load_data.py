...
# # 2. 산업군별 종목 리스트 로드
# def get_corp_in_index(index_name, year, quarter) -> list:
#     '''
#     산업군 이름, 연도, 분기를 입력하면 해당 산업군 내에 들어있는 티커 리스트를 반환
#     '''
#     try:
#         corp_in_index_path = os.path.join(corp_in_index_path, year)
#         file_name_list = []

#         # 지정한 경로 내의 모든 파일 이름 조회
#         for file_name in os.listdir(corp_in_index_path):
#             full_path = os.path.join(corp_in_index_path, file_name)
#             if os.path.isfile(full_path):  # 파일인지 확인
#                 # _corp_list.csv를 제거하고 앞부분만 가져오기
#                 cleaned_name = file_name.replace('_corp_list.csv', '')
#                 file_name_list.append(cleaned_name)

#         if index_name in file_name_list:
#             index_list_df_path = os.path.join(corp_in_index_path, f'{index_name}_corp_list.csv')
#             index_list_df = pd.read_csv(index_list_df_path)
#             index_list = list(index_list_df['stock_code'])
#             index_list = [str(index).zfill(6) for index in index_list]
                
#             return index_list

#         else:
#             print(f'조회하려는 인덱스({index_name})가 DB 내에 존재하지 않습니다.')
#             return None
        
#     except Exception as e:
#         print(f'get_corp_in_index에서 오류 발생: {e}')
#         return None
    
# # 3. 인덱스 이름을 티커로 변환하는 함수
# def get_index_ticker(index_name, year, quarter) -> str:
#     '''
#     인덱스 이름을 넣으면 인덱스 티커로 반환하는 함수
#     '''
#     date = ''
#     if quarter == 'Q1':
#         date = f'{year}0101'
...
#     try:
#         tickers = stock.get_index_ticker_list(date)
#         for ticker in tickers:
#             temp_index_name = stock.get_index_ticker_name(ticker)
#             if index_name == temp_index_name:
#                 return ticker
#     except Exception as e:
#         print(f'{index_name}에 해당하는 인덱스의 종목 코드를 찾을 수 없습니다.')
#         return None

# 4. 인덱스 지표의 OHLCV 데이터 로드
